...
#'
#'@note This function is based on code from the paper Graff, A., Chousou-Polydouri, N., Inman, D., Skirgård, H., Lischka, M., Zakharko, T., Barbieri, C., and Bickel, B., (2025). Curating global datasets of structural linguistic features for independence.Scientific Data . Original code can be found here: https://github.com/annagraff/crossling-curated/blob/main/scripts/GBI/build-datasets.R. The function rgrambank::make_GBI has been modified by Hedvig Skirgård to adapt to the rgrambank package and take into account changes between Grambank v1 and v2. The modifications are: turn binarised features (in Grambank v2 and further versions) corresponding multistate, rename variables to avoid loading recode-patterns several times, remove language meta-data, remove lazy loading of variables for dplyr::filter + dplyr::mutate, replace data.table::setDT with base::as.data.frame, replace spread with pivot_wider and other updates to more recent versions of tidyverse packages.
#' Regarding binarisation: if the input ValueTable contains binarised features 
#' (GB024a/b, GB025a/b, GB065a/b, GB130a/b, GB193a/b, GB203a/b) introduced in 
#' Grambank v2, these are automatically reconstructed into their multistate 
#' equivalents before processing. If both multistate and native binary codings 
#' exist for the same language, native binary takes priority and is used to render
...
if(any(c("GB024a",
  ...
  first_set_rec <- dplyr::rowwise(first_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE) |>  
    as.data.frame() |>
    .check_no_listcols("first_set_rec")
  ...
  second_set_rec <- dplyr::rowwise(second_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE) |>  
    as.data.frame() |>
    .check_no_listcols("second_set_rec")
  ...
  third_set_rec <- dplyr::rowwise(third_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE)  |>  
    as.data.frame() |>
    .check_no_listcols("third_set_rec")
  ...
  fourth_set_rec <- dplyr::rowwise(fourth_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE) |>  
    as.data.frame() |>
    .check_no_listcols("fourth_set_rec")
  ...
  fifth_set_rec <- dplyr::rowwise(fifth_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE) |>  
    as.data.frame() |>
    .check_no_listcols("fifth_set_rec")
  ...
  sixth_set_rec <- dplyr::rowwise(sixth_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE) |>  
    as.data.frame() |>
    .check_no_listcols("sixth_set_rec")
  ...
  seventh_set_rec <- dplyr::rowwise(seventh_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE)  |>  
    as.data.frame() |>
    .check_no_listcols("seventh_set_rec")
  ...
  eighth_set_rec <- dplyr::rowwise(eighth_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE)  |>  
    as.data.frame() |>
    .check_no_listcols("eighth_set_rec")
  ...
  ninth_set_rec <- dplyr::rowwise(ninth_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE)  |>  
    as.data.frame() |>
    .check_no_listcols("ninth_set_rec")
  ...
  tenth_set_rec <- dplyr::rowwise(tenth_set) |> dplyr::do({
    ...
    tidyr::pivot_wider(names_from = "feature", values_from = "value", names_sort = TRUE) |>  
    as.data.frame() |>
    .check_no_listcols("tenth_set_rec")
  ...
  for (rd in seq_len(nrow(rds))){
    id <- rds |> 
      dplyr::slice(rd) |> 
      dplyr::pull(dplyr::all_of("modification.ID"))
...
# this function guards against tidyr::pivot_wider() silently producing
# list-columns, which happens when there are duplicate id x names_from
# combinations. tidyr::spread() (which pivot_wider() replaces throughout this
# file) used to error loudly in that situation; pivot_wider() does not, by
# design, so we replicate the old fail-fast behaviour explicitly here.
.check_no_listcols <- function(df, label){
  testthat::expect_true(
    !any(vapply(df, is.list, logical(1))),
    info = paste0(
      "tidyr::pivot_wider() produced list-column(s) in '", label, 
      "' -- check for duplicate Language_ID x feature combinations in the input data."
    )
  )
  invisible(df)
}
