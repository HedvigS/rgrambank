...
# Future releases of Grambank may contain a mix of multistate features and binarised features already in the release. In the functions here, if the dataset already contains binarised features, that is referred to as "binary binary". Users can choose to disregard these, or only use these. If the users selects keep_binary_binary = TRUE AND trim_to_only_binary_binary = FALSE this will result in a mix of binary binarised features and multistate features which have been binarised. For more information, see: https://github.com/grambank/grambank/wiki/Binarised-features.
...
#devtools::install_version("tidyverse", version = "2.0.0", repos = "http://cran.us.r-project.org")
library(tidyverse)
